...
const AdminBillsDetail = () => {
  ...
  const fetchCustomerDetails = async (e) => {
    ...
    try {
      ...
      const response = await fetch(`http://localhost:8686/orders/user/${id}`, {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${accessToken}`,
          ...
          <form onSubmit={fetchCustomerDetails}>
            <div className="flex flex-col gap-[50px]">
              ...
              <div className="flex flex-row justify-between w-full">
                <DetailRow2
                  label="Tên hàng"
                  value={orderedProduct.slice(0,5)
                ...
                <div className="flex flex-col gap-[10px]">
                  ...
                  <div className="flex flex-row gap-[20px]">
                    {orderedProduct.slice(0, 5).map((product) => (
                      <div>
                        <div
                    ...
                    {orderedProduct.length > 5 && <p>And {orderedProduct.length - 5} more</p>}
                  </div>
                </div>
              </div>
            </div>
